...
Character Development Consultant (CDC)
---
...
- Parse the Story Foundation's cast list (`9_principal_characters` in the Project 1 JSON export) and the Story Spine (`11_story_spine`).
- Silently classify each named character into: Critical / Major / Supporting / Minor / Incidental, per **Appendix A (Character Priority Budget)** — an engineering-defined heuristic. **Correction (2026-07-23):** the CDRM (`system-prompts/sp02-character-development-reference-manual#.md`) has no "Character Priority Budget" section — its actual Section 2 is "Character Architecture & the Triad" (Want/Personality/Values), and no priority-tier or depth-tier taxonomy exists anywhere in the document. This requirement's original citation was wrong; Appendix A below is the real source of truth for this classification until/unless the author supplies an actual Priority Budget definition.
...
- **Correction (2026-07-23):** the CDRM's actual Character Triad (§2) is **Want (Motivation) / Personality-How (observable behavior, risk tolerance, communication) / Values (Why)** — not "Want, Need, Values" as originally stated here. `Need` is a related but distinct concept the CDRM introduces separately (the unacknowledged internal transformation the Want's pursuit forces the character to confront), not a Triad member. The system must extract and hold all **four** as distinct, separately-confirmed fields: Want (external), Personality/How (observable behavior), Need (internal transformation), and Values (uncrossable lines). The original three-field requirement silently dropped Personality/How — this restores it.
...
## Appendix A: Character Priority Budget (Engineering-Defined Heuristic)

**Added 2026-07-23.** The CDRM does not define a priority/depth taxonomy despite §5.1/§5.2 assuming one exists (see the correction note in §5.1). This heuristic is engineering-owned — analogous to Project 1's author-type classifier (heuristic, not sourced from an external spec) — and should be revised if the author later supplies a real Character Priority Budget definition.

Classification uses signals from the ingested Project 1 JSON export (`8_dramatic_engine`, `9_principal_characters`, `11_story_spine`):

| Tier | Classification rule | Interview depth | CDRM stage coverage |
| --- | --- | --- | --- |
| **Critical** | Matches `dramatic_engine.protagonist` or `antagonistic_force`, OR named in ≥3 Story Spine beats | Exhaustive | Full 6-stage interview, all 8 Bible sections developed in depth |
| **Major** | `story_role` indicates deuteragonist / co-protagonist / mentor / love-interest, OR named in ≥1 Story Spine beat | Comprehensive | Full 6-stage interview, lighter pass on Ensemble Interconnection & Milestone Arc Timeline |
| **Supporting** | In `principal_characters` with a plot-tied `primary_function`, no Story Spine presence | Standard | Streamlined pass through all 6 stages, ~1 question per stage |
| **Minor** | In `principal_characters` with only a one-line function, no Story Spine presence | Basic | Position & Purpose + a light Psychological Core pass only; Relationship Integration and Arc Pacing skipped |
| **Incidental** | Not in `principal_characters` at all — surfaces only when the author mentions them in conversation | Reference | No 6-stage interview run; name/role/one-line function captured verbatim and the character is marked `Deferred` |

Matrix recomputation (per §5.1) re-runs this table whenever the author edits or adds a character mid-session — a character can move tiers (e.g. a Minor character the author starts developing heavily gets promoted to Supporting) but only via an explicit author action, never silently.