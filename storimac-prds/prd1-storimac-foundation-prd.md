# PRD: SDOS Project 1 — Story Development Interview System (SDIS)
...
- Output: exactly 1 Primary Format + 0–2 Supporting Formats (hard cap of 3 total), each identified by its Title (e.g., `The Chosen Burden`) and one sentence of diagnostic reasoning tying it to the author's stated premise. The format's catalog Volume/Code (e.g. `A05`) is tracked internally as retrieval-grounding metadata and for the Stage 7 Common Mistakes lookup — it must never be surfaced to the author in conversation, in the compiled Story Foundation Document, or in any downstream-generated document/script (Projects 2–5).
...
- **State store:** Postgres in production, SQLite acceptable for local/dev or single-user deployments. One row per element per Project, plus a `projects` table and a `messages` table for transcript. **Deviation flagged (2026-07-23):** production deployment target is now Firebase + Google Cloud Run; the state store is Firestore, not Postgres. See `ARCHITECTURE.md` §6 for the collection-based data model and how dependency/cascade queries (Projects 3–4) map onto Firestore's `array-contains` queries instead of SQL joins.
...
{
  "updates": [
    {"element_id": "primary_format", "status": "Confirmed", "value": "The Chosen Burden", "retrieval_code": "A05"},
    {"element_id": "supporting_formats", "status": "Working", "value": ["The Last Stand"], "retrieval_code": ["B08"]}
  ],
...
- `retrieval_code` is internal-only: it feeds the automated grounding check (§11) and the Stage 7 Common Mistakes lookup (§5.6). It is never rendered in a chat reply, never included in the compiled Story Foundation Document (§10.2 intentionally omits it), and must not be echoed into any Project 2–5 output — `value` (the format's Title) is the only author-facing representation.
...
```json
{
  ...
  "3_story_format": {"primary_format": {"name":"", "reason":""}, "supporting_formats": [{"name":"", "reason":""}]},
  "4_premise": "",
  "5_logline": "",
...
**Note on format codes:** `3_story_format` intentionally has no `code` field. The 101 Story Formats catalog code (e.g. `A05`) that Stage 2 diagnoses against is retrieval/grounding metadata (§6.3, §11) and a Stage 7 lookup key (§5.6) — it is not part of this export contract and must never appear in the compiled document, in conversation, or in anything Projects 2–5 generate from this data. Only `name` (the format's Title) and `reason` are author- and downstream-facing.
...
- **Format-diagnosis grounding:** % of Stage 2 diagnoses whose internally-tracked `retrieval_code` (§6.2 — never author-facing) is a real, retrieved format code vs. a hallucinated one (should be 100%; add an automated check that every such code exists in the source index).
