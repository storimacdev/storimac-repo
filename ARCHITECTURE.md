...
- **Export pipelines:** Markdown/JSON generation is native (template-fill, see §2), no library needed. `.docx` export (needed by Projects 2, 3, 4) uses the `docx` npm package — settled by precedent (2026-09-10, issue #54): already in production for Project 2's export (`web/src/lib/docx/characterBibleDocx.ts`); `docxtemplater` was the other option under consideration but Project 3/4's `.docx` work should reuse `docx` rather than introduce a second templating library for one project.
...
- **Security:** Firebase Security Rules keying off `resource.data.ownerUid == request.auth.uid` (checked on the parent `Story` doc for subcollection reads/writes) started as a single-author-per-story model — arguably simpler than hand-rolling ownership checks in API route middleware, which was a genuine point in Firestore's favor once Auth became Firebase too. **Correction (2026-09-10, issue #54):** this line was stale — the app has since grown real multi-member workspace membership (`web/src/lib/workspace/workspaceStore.ts`, `/api/workspaces/[workspaceId]/members/[uid]` and `/invites` routes), with `getMembership` checked on every Project 2/3 route already. The data model was never a hard single-user assumption in practice; this section just hadn't been updated to say so.
...
- ~~`.docx` generation library choice: leaning `docx` (npm) or `docxtemplater` per §4/§6, not yet finalized between the two.~~ **Settled (2026-09-10, issue #54):** `docx` (npm) — already proven in Project 2's export; Project 3/4 reuse it rather than introducing `docxtemplater` for one project. See §4.
